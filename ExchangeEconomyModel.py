...
class ExchangeEconomyModelClass:
    ...
    def solve_walras(self,p_guess,print_output=True, method='tatonnement'):
        ...
        p1 = float(p_guess) # initial price guess
        ...
        while True:
            ...
            eps1, eps2 = self.check_market_clearing(p1) # excess demand

            sol.p_path.append(p1)
            sol.eps1_path.append(eps1)

            if k >= par.K: # maximum number of iterations exceeded
                raise ValueError('Max iterations exceeded (k >= par.K)')

            if np.abs(eps1) < par.tol: # convergence criterion met

                sol.p1 = p1 
                sol.p_path = np.array(sol.p_path) 
                sol.eps1_path = np.array(sol.eps1_path)

                x1A, x2A = self.demand_A(p1) 
                sol.xA = (x1A, x2A)
                sol.uA = self.utility_A(x1A, x2A)
                sol.uB = self.utility_B(1 - x1A, 1 - x2A)

                if print_output: # print results
                    print("\nSolved!")
                    print(f" {k:3d}: p1 = {p1:12.8f}, x1A = {x1A:12.8f}, x2A = {x2A:12.8f}")
                    print(f" Excess demand of good 1: {eps1:14.8e}")
                    print(f" Excess demand of good 2: {eps2:14.8e}")
                break
            ...
            if print_output:
                ...
                if k < 5 or k%5 == 0:
                    x1A, x2A = self.demand_A(p1)
                    x1B, x2B = self.demand_B(p1)
                    print(f"{k:3d}: p1 = {p1:12.8f} -> excess demand good 1 -> {eps1:14.8e}", end="")
                    print(f", x1A = {x1A:12.8f}, x2A = {x2A:12.8f}", end="")
                    print(f", x1B = {x1B:12.8f}, x2B = {x2B:12.8f}")
                elif k == 5:
                    print('   ...')

            p1 = p1 + par.nu * eps1 # update price guess
